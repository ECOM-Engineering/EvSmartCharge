...
def popSearchCharger(ip ='http://192.168.0.17'):
    """
    :param batteryLevel:    actual level in %
    :param chargeLimit:     max level to become charged in %
    :param currentSet:      charge current in A
    :param currentLimit:    max allowed charge current
    :param phases:        True, if 3 phase charging is possible
    :return:                Dict {'cancelled', 'currentSet', 'chargeLimit', '3_phases'}
    """
    layout_popC = [[sg.Text('CHARGER NOT FOUND at!', pad=0), sg.Text(' 2nd Text')],
                   [sg.Text('Not found at:', size=10), sg.Input(ip, size=30,key='-BAD_IP-',disabled=True )],
                   ...
                   [sg.Button('Cancel'), sg.Button('SEARCH!', focus=True)]]
    ...
    while True:
        ...
        if ev2 == sg.WIN_CLOSED or ev2 == 'Cancel':
           #    print(manualSettings)
            break
        if ev2 == 'SEARCH!':
            foundIP = search_charger('http://192.168.0.')
